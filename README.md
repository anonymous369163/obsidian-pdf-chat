...
**Save answer** appends visible user/assistant content to `PDF Chat/Reading Notes/<paper>.md`; multi-paper turns go to `Synthesis.md`. **Export Markdown** writes that individual question and answer to a timestamped file under `PDF Chat/Exports/<discussion>/`, including the API/Codex mode and Obsidian links to the current and referenced PDFs. The optional selected passage is excluded by default and is included only when **Save selected passage text** is enabled. Session export writes the full visible discussion to `PDF Chat/Exports/` without mutating the session. All note and export writes are serialized per path, and exported evidence is sanitized before writing, so retrying a failed write cannot partially overwrite an existing note or bypass the local-data privacy boundary.
